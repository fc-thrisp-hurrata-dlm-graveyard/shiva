...
func (c *color) SetHex(value uint) {
...
func (c *color) AddScalar(s float32) {
	r := c.R() + s
	g := c.G() + s
	b := c.B() + s
	c.Set(r, g, b)
...
func (c *color) MulScalar(s float32) {
	r := c.R() * s
	g := c.G() * s
	b := c.B() * s
	c.Set(r, g, b)
...
//func (c *color) Lerp(o Color, alpha float32) {
//	r := c.R() + (o.R()-c.R())*alpha
//	g := c.G() + (o.G()-c.G())*alpha
//	b := c.B() + (o.B()-c.B())*alpha
//	c.Set(r, g, b)
...
func (c *color) SetKey(k string) {
	c.SetHex(colorKey[k])
}

var colorKey = map[string]uint{
	"aliceblue":            0xF0F8FF,
