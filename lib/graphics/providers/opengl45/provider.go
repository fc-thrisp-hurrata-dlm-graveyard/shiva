...
//
func (g *OGL45) ClearDepth(v float32) {
	gl.ClearDepthf(v)
}

//
func (g *OGL45) ClearStencil(v int32) {
	gl.ClearStencil(v)
}
...
//
func (g *OGL45) GetAttribCurrentLocation(name string) int32 {
	glName := name + "\x00"
	return gl.GetAttribLocation(uint32(g.currentProgram), gl.Str(glName))
}
...
func (g *OGL45) GetUniformCurrentLocation(name string) int32 {
	glName := name + "\x00"
